...
async function openApp(appName: string): Promise<void> {
    ...
    if (iconElement) {
        ...
        if(span) title = span.textContent || appName;
         ...
         switch(appName) {
            ...
            case 'displayProperties': iconSrc = 'https://win98icons.alexmeub.com/icons/png/display_properties-1.png'; title = 'Display'; break;
         }
    }
    ...
    else if (appName === 'displayProperties') {
        initDisplayProperties(windowElement);
    }
}
...
function initAiBrowser(windowElement: HTMLDivElement): void {
    ...
    const setLoadingText = (text: string) => {
        ...
        if (p) p.innerHTML = `${text}<span class="dialup-animation"><span class="dot">.</span><span class="dot">.</span><span class="dot">.</span></span>`;
    };
    ...
    async function navigateToUrl(url: string): Promise<void> {
        ...
        try {
            ...
            try {
                ...
                setLoadingText(`Generating 90s layout for ${domain}`);
                const websiteAndImagePrompt = `
                Create a complete 90s-style website for the domain "${domain}".
                MUST include: garish 90s styling (neon, comic sans, tables), content specific to "${domain}", scrolling marquee, retro emoji/ascii, blinking text, visitor counter (9000+), "Under Construction" signs. Fun, humorous, 1996 feel. No modern design.
                The website should have one relevant image. Provide the HTML for the website with a placeholder "<!-- IMAGE_HERE -->" where the image should go.
                After the HTML, on a new line, provide a detailed, descriptive prompt for an AI image generator to create the image that would fit the website's theme. Start this description with "IMAGE_PROMPT:".
                `;
                // @ts-ignore
                const textResult = await geminiInstance.models.generateContent({ model: 'gemini-2.5-flash', contents: websiteAndImagePrompt });
                const responseText = textResult.text;
                const parts = responseText.split('IMAGE_PROMPT:');
                let htmlContent = parts[0].replace(/```html|```/g, '').trim();
                const imagePrompt = parts[1] ? parts[1].trim() : `A retro 90s style image for a website about ${domain}`;

                setLoadingText(`Generating retro image for ${domain}`);
                // @ts-ignore
                const imageResult = await geminiInstance.models.generateImages({
                ...
                if (htmlContent.includes('<!-- IMAGE_HERE -->')) {
                    htmlContent = htmlContent.replace('<!-- IMAGE_HERE -->', `<img src="${imageUrl}" alt="Site Image" style="max-width:80%; display:block; margin:10px auto; border: 3px ridge gray;">`);
                } else { // Fallback insertion
                    htmlContent = htmlContent.replace(/(<\/h1>)/i, `$1\n<img src="${imageUrl}" alt="Site Image" style="max-width:80%; display:block; margin:10px auto; border: 3px ridge gray;">`);
                }
                ...
                iframe.src = 'data:text/html;charset=utf-8,' + encodeURIComponent(`<html><body><h1>Connection Error!</h1><p>Could not generate the website for ${domain}.</p><p>Error: ${e.message}</p></body></html>`);
            } finally {
...
async function critiquePaintDrawing(): Promise<void> {
    ...
    try {
        ...
        const textPart = { text: prompt };
         // @ts-ignore
        const result = await geminiInstance.models.generateContent({ model: "gemini-2.5-flash", contents: { parts: [textPart, imagePart] } });
...
updateClock();
setInterval(updateClock, 1000 * 30); // Update every 30 seconds is enough